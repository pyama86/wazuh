...
static int wm_vuldet_xml_parser(OS_XML *xml, XML_NODE node, wm_vuldet_db *parsed_oval,
                                update_node *update, vu_logic condition);
static int wm_vuldet_json_parser(cJSON *json_feed, wm_vuldet_db *uparsed_vulnerabilities,
                                 update_node *update);
...
static const char *wm_vuldet_decode_package_version(char *raw, const char **OS, char **package_name,
...
static int wm_vuldet_get_software_info(agent_software *agent, sqlite3 *db, OSHash *agents_triag,
                                       unsigned long ignore_time);
static int wm_vuldet_report_agent_vulnerabilities(agent_software *agents, sqlite3 *db, int max);
static int wm_vuldet_check_agent_vulnerabilities(agent_software *agents, OSHash *agents_triag,
                                                 unsigned long ignore_time);
static int wm_checks_package_vulnerability(char *version, const char *operation,
...
static const char *wm_vuldet_set_oval(const char *os_name, const char *os_version,
                                      update_node **updates, distribution *wm_vuldet_set_oval);
...
int *vu_queue;
const wm_context WM_VULNDETECTOR_CONTEXT = {"vulnerability-detector", (wm_routine)wm_vuldet_main,
                                            (wm_routine)wm_vuldet_destroy,
                                            (cJSON * (*)(const void *)) wm_vuldet_dump};

const char *vu_dist_tag[] = {"UBUNTU",  "DEBIAN",   "REDHAT", "CENTOS", "AMAZLINUX", "WINDOWS",
                             "MACOS",   "PRECISE",  "TRUSTY", "XENIAL", "BIONIC",    "JESSIE",
                             "STRETCH", "WHEEZY",   "RHEL5",  "RHEL6",  "RHEL7",     "WXP",
                             "W7",      "W8",       "W81",    "W10",    "WS2008",    "WS2008R2",
                             "WS2012",  "WS2012R2", "WS2016", "MACOSX", "UNKNOWN"};

const char *vu_dist_ext[] = {"Ubuntu",
                             "Debian",
...
const char *wm_vuldet_set_oval(const char *os_name, const char *os_version, update_node **updates,
                               distribution *agent_dist)
{
  ...
  for (i = 0; i < OS_SUPP_SIZE; i++) {
    if (updates[i] && updates[i]->allowed_OS_list) {
      ...
      char *allowed_os;
      char *allowed_ver;
      for (allowed_os = *updates[i]->allowed_OS_list, allowed_ver = *updates[i]->allowed_ver_list,
          j = 0;
           allowed_os; ++j) {
...
{
  int need_update = 1;
  return wm_vuldet_fetch_feed(upd, dist, &need_update) ||
         (need_update && wm_vuldet_update_feed(upd));
}
...
int wm_vuldet_compare(char *version_it, char *cversion_it)
{
  ...
  if (strcmp(version_it, cversion_it)) {
    for (it = 0, i = 0, j = 0, version_found = 0, cversion_found = 0; it < VU_MAX_VER_COMP_IT;
         it++) {
      if (!version_found) {
        if (version_it[i] == '\0') {
          version_found = 3;
    ...
    while (sql_result = wm_vuldet_step(stmt), sql_result == SQLITE_ROW) {
      ...
      *condition = '\0';
      if (pending) {
        ...
        if (v_type = wm_checks_package_vulnerability(version, operation, operation_value),
            v_type == OS_INVALID) {
          goto error;
        }
        if (v_type == VU_NOT_FIXED) {
          ...
          mtdebug2(WM_VULNDETECTOR_LOGTAG, VU_NOT_VULN, package, agents_it->agent_id, cve, version,
                   operation, operation_value);
          continue;
        } else if (v_type == VU_ERROR_CMP) {
          mtdebug1(WM_VULNDETECTOR_LOGTAG,
                   "The '%s' and '%s' versions of '%s' package could not be compared. Possible "
                   "false positive.",
                   version, operation_value, package);
          snprintf(condition, OS_SIZE_1024, "Could not compare package versions (%s %s).",
                   operation, operation_value);
        } else {
          snprintf(state, 15, "Fixed");
          if (!second_operation || *second_operation == '0') {
            mtdebug2(WM_VULNDETECTOR_LOGTAG, VU_PACK_VER_VULN, package, agents_it->agent_id, cve,
                     version, operation, operation_value);
          } else {
            // The first condition is vulnerable, but the second also has to be
            if (v_type = wm_checks_package_vulnerability(version, second_operation,
                                                         second_operation_value),
                v_type == OS_INVALID) {
              goto error;
            } else if (v_type == VU_VULNERABLE) {
              mtdebug2(WM_VULNDETECTOR_LOGTAG, VU_DOUBLE_VULN, package, agents_it->agent_id, cve,
                       version, operation, operation_value, second_operation,
                       second_operation_value);
            } else {
              mtdebug2(WM_VULNDETECTOR_LOGTAG, VU_DOUBLE_NOT_VULN, package, agents_it->agent_id,
                       cve, version, operation, operation_value, second_operation,
                       second_operation_value);
              continue;
            }
          }
      ...
      if (alert_cve = cJSON_CreateObject(), alert_cve) {
        ...
        if (cwe)
          cJSON_AddStringToObject(alert_cve, "cwe_reference", cwe);
        if (bugzilla_reference)
          cJSON_AddStringToObject(alert_cve, "bugzilla_reference", bugzilla_reference);
        if (reference) {
      ...
      if (agents_it->agent_ip) {
        snprintf(header, OS_SIZE_256, VU_ALERT_HEADER, atoi(agents_it->agent_id),
                 agents_it->agent_name, agents_it->agent_ip);
...
int wm_vuldet_check_agent_vulnerabilities(agent_software *agents, OSHash *agents_triag,
                                          unsigned long ignore_time)
{
  agent_software *agents_it;
  sqlite3 *db;
  ...
  for (i = 1, agents_it = agents;; i++) {
    ...
    if (result = wm_vuldet_get_software_info(agents_it, db, agents_triag, ignore_time),
        result == OS_INVALID) {
    ...
    if (result != 2) { // There exists packages for the agent
      if (VU_AGENT_REQUEST_LIMIT && i == VU_AGENT_REQUEST_LIMIT) {
        ...
        i = 0;
        if (sqlite3_prepare_v2(db, vu_queries[VU_REMOVE_AGENTS_TABLE], -1, &stmt, NULL) !=
            SQLITE_OK) {
          return wm_vuldet_sql_error(db, stmt);
        }
        if (wm_vuldet_step(stmt) != SQLITE_DONE) {
...
int wm_vuldet_insert(wm_vuldet_db *parsed_oval)
{
  ...
  while (vul_it) {
    // If you do not have this field, it has been discarded by the preparser and the OS is not
    // affected
    if (vul_it->state_id) {
...
char *wm_vuldet_xml_preparser(char *path, distribution dist)
{
  ...
  char *found;
  char *tmp_file;
  static const char *exclude_tags[] = {// Debian
  ...
  while (fgets(buffer, OS_MAXSTR, input)) {
    if (dist == DIS_UBUNTU) { // 5.11.1
      ...
      case V_OBJECTS:
        ...
        break;
      case V_DEFINITIONS:
        if ((found = strstr(buffer, "is not affected")) && (found = strstr(buffer, "negate")) &&
            strstr(found, "true")) {
          continue;
      ...
      case V_OBJECTS:
        ...
        break;
      case V_DEFINITIONS:
        if (strstr(buffer, exclude_tags[0]) || strstr(buffer, exclude_tags[1])) {
          continue;
        } else if (found = strstr(buffer, "</definitions>"), found) {
          state = V_STATES;
        }
  ...
  if (advisories) {
    for (; advisories; advisories = advisories->next) {
      if (!advisories_str) {
        ...
        size =
            snprintf(str_it, strlen(advisories->valuestring) + 2, ",%s", advisories->valuestring) +
            size;
      }
    }
  }
...
int wm_vuldet_xml_parser(OS_XML *xml, XML_NODE node, wm_vuldet_db *parsed_oval, update_node *update,
                         vu_logic condition)
{
  int i, j;
  int retval = 0;
  ...
  for (i = 0; node[i]; i++) {
    ...
    if ((dist == DIS_UBUNTU && !strcmp(node[i]->element, XML_DPKG_LINUX_INFO_STATE)) ||
      ...
      if (node[i]->attributes) {
        ...
        if (!parsed_oval->info_states->operation && !strcmp(*node[i]->attributes, XML_DATATYPE) &&
            !strcmp(*node[i]->values, "version")) {
      ...
      for (j = 0; node[i]->attributes[j]; j++) {
        if (!strcmp(node[i]->attributes[j], XML_CLASS)) {
          if (!strcmp(node[i]->values[j], XML_VULNERABILITY)) {
            ...
            if (wm_vuldet_xml_parser(xml, chld_node, parsed_oval, update, condition) ==
                OS_INVALID) {
              retval = OS_INVALID;
              goto end;
            }
      ...
      if (!node[i]->attributes) {
        ...
        for (j = 0; node[i]->attributes[j]; j++) {
          if (!strcmp(node[i]->attributes[j], XML_OPERATOR)) {
            ...
            if (!strcmp(node[i]->values[j], XML_OR)) {
              if (chld_node = OS_GetElementsbyNode(xml, node[i]), !chld_node) {
                continue;
              } else if (result = wm_vuldet_xml_parser(xml, chld_node, parsed_oval, update, VU_OR),
                         result == OS_INVALID) {
                retval = OS_INVALID;
                goto end;
              }
              ...
              if (chld_node = OS_GetElementsbyNode(xml, node[i]), !chld_node) {
                continue;
              } else if (result = wm_vuldet_xml_parser(xml, chld_node, parsed_oval, update, VU_AND),
                         result == OS_INVALID) {
                retval = OS_INVALID;
                goto end;
              }
        ...
        if (!operator_found && node[i]->attributes && !strcmp(*node[i]->attributes, XML_COMMENT) &&
            !strcmp(*node[i]->values, "file version")) {
          if (chld_node = OS_GetElementsbyNode(xml, node[i]), !chld_node) {
            continue;
          } else if (wm_vuldet_xml_parser(xml, chld_node, parsed_oval, update, VU_AND) ==
                     OS_INVALID) {
            retval = OS_INVALID;
            goto end;
          }
      ...
      os_strdup(node[i]->content, parsed_oval->info_cves->published);
               ...
               !strcmp(node[i]->element, XML_TESTS) || !strcmp(node[i]->element, XML_STATES) ||
               !strcmp(node[i]->element, XML_ADVIDSORY) || !strcmp(node[i]->element, XML_DEBIAN) ||
      ...
      if (chld_node = OS_GetElementsbyNode(xml, node[i]), !chld_node) {
        goto invalid_elem;
      } else if (wm_vuldet_xml_parser(xml, chld_node, parsed_oval, update, condition) ==
                 OS_INVALID) {
        retval = OS_INVALID;
        goto end;
      }
...
const char *wm_vuldet_decode_package_version(char *raw, const char **OS, char **package_name,
                                             char **package_version)
{
  ...
  if (sqlite3_open_v2(CVE_DB, &db, SQLITE_OPEN_READWRITE, NULL) != SQLITE_OK) {
    ...
    if (wm_vuldet_step(stmt) == SQLITE_ROW) {
      ...
      for (i = 0; stored_timestamp[i] != '\0'; i++) {
        if (stored_timestamp[i] == '-' || stored_timestamp[i] == ' ' ||
            stored_timestamp[i] == ':' || stored_timestamp[i] == 'T') {
          continue;
        }
  ...
  *need_update = 1;
  ...
  if (update->dist_ref == DIS_REDHAT) {
    ...
    if (*repo != '\0') {
      ...
      while (page) {
        if (!attempt) {
          snprintf(repo, OS_SIZE_2048, RED_HAT_REPO, update->update_from_year,
                   RED_HAT_REPO_REQ_SIZE, page);
        ...
        if (fp = fopen(CVE_TEMP_FILE, "r"), !fp) {
          mtdebug1(WM_VULNDETECTOR_LOGTAG, "tempfile open error error", OS);
          goto end;
        }
  ...
  for (attempts = 0;; attempts++) {
    if (!wurl_request(repo, CVE_TEMP_FILE)) {
      ...
      mtdebug1(WM_VULNDETECTOR_LOGTAG, "down attempls error", OS);
      goto end;
    }
  ...
  if (fp = fopen(CVE_TEMP_FILE, "r"), !fp) {
    mtdebug1(WM_VULNDETECTOR_LOGTAG, "tmp file open error", OS);
    goto end;
  }
  ...
  while (fgets(buffer, OS_MAXSTR, fp)) {
    if (found = strstr(buffer, timestamp_tag), found) {
      ...
      if (close_tag = strstr(found, "<"), !close_tag) {
        mtdebug1(WM_VULNDETECTOR_LOGTAG, "tag error", OS);
        goto end;
      }
      *close_tag = '\0';
  ...
  if (wm_vuldet_check_update_period(upd)) {
    ...
    if (wm_vuldet_check_update(upd, dist_tag)) {
      if (!upd->attempted) {
        ...
        upd->attempted = 1;
        mtdebug1(WM_VULNDETECTOR_LOGTAG, VU_UPDATE_RETRY, upd->dist, upd->version,
                 (long unsigned)WM_VULNDETECTOR_RETRY_UPDATE);
      } else {
...
int wm_vuldet_updatedb(update_node **updates)
{
  ...
  if (wm_vuldet_run_update(updates[CVE_BIONIC], vu_dist_tag[DIS_BIONIC], vu_dist_ext[DIS_BIONIC]) ||
      ...
      wm_vuldet_run_update(updates[CVE_PRECISE], vu_dist_tag[DIS_PRECISE],
                           vu_dist_ext[DIS_PRECISE]) ||
      // Debian
      wm_vuldet_run_update(updates[CVE_STRETCH], vu_dist_tag[DIS_STRETCH],
                           vu_dist_ext[DIS_STRETCH]) ||
...
int wm_vuldet_json_parser(cJSON *json_feed, wm_vuldet_db *parsed_vulnerabilities,
                          update_node *update)
{
  cJSON *json_it;
  cJSON *cve_content;
  ...
  if (ref == DIS_REDHAT) {
    for (json_it = json_feed->child; json_it; json_it = json_it->next) {
      ...
      if (tmp_affected_packages) {
        ...
        parsed_vulnerabilities->info_cves->reference =
            wm_vuldet_build_url(VU_BUILD_REF_CVE_RH, tmp_cve);
        w_strdup(tmp_bugzilla_description, parsed_vulnerabilities->info_cves->description);
        parsed_vulnerabilities->info_cves->cvss =
            (tmp_cvss_score != -1) ? w_double_str(tmp_cvss_score) : NULL;
        parsed_vulnerabilities->info_cves->cvss3 =
            (tmp_cvss3_score != -1) ? w_double_str(tmp_cvss3_score) : NULL;
        w_strdup(tmp_cvss_scoring_vector, parsed_vulnerabilities->info_cves->cvss_vector);
        parsed_vulnerabilities->info_cves->bugzilla_reference =
            wm_vuldet_build_url(VU_BUILD_REF_BUGZ, tmp_bugzilla);
        parsed_vulnerabilities->info_cves->advisories =
            wm_vuldet_extract_advisories(tmp_advisories);
        ...
        for (; tmp_affected_packages; tmp_affected_packages = tmp_affected_packages->next) {
          ...
          if (!wm_vuldet_decode_package_version(
                  tmp_affected_packages->valuestring,
            ...
            mterror(WM_VULNDETECTOR_LOGTAG, VU_VER_EXTRACT_ERROR,
                    parsed_vulnerabilities->rh_vulnerabilities->package_name);
            continue;
...
int wm_vuldet_get_software_info(agent_software *agent, sqlite3 *db, OSHash *agents_triag,
                                unsigned long ignore_time)
{
  int sock = 0;
  unsigned int i;
  ...
  for (i = 0; i < VU_MAX_WAZUH_DB_ATTEMPS &&
              (sock = OS_ConnectUnixDomain(WDB_LOCAL_SOCK_PATH, SOCK_STREAM, OS_SIZE_6144)) < 0;
       i++) {
    mterror(WM_VULNDETECTOR_LOGTAG, "Unable to connect to socket '%s'. Waiting %d seconds.",
            WDB_LOCAL_SOCK_PATH, i);
    sleep(i);
  }
  ...
  if (OS_SendSecureTCP(sock, size + 1, buffer) ||
      (size = OS_RecvSecureTCP(sock, buffer, OS_SIZE_6144)) < 1) {
    close(sock);
  ...
  size = snprintf(scan_id, OS_SIZE_1024, "%i",
                  (int)cJSON_GetObjectItem(obj, "data")->child->child->valuedouble);
  scan_id[size] = '\0';
  ...
  i = 0;
  size = snprintf(buffer, OS_SIZE_6144, vu_queries[request], agent->agent_id, scan_id,
                  VU_MAX_PACK_REQ, i);
  if (OS_SendSecureTCP(sock, size + 1, buffer)) {
...
void *wm_vuldet_main(wm_vuldet_t *vulnerability_detector)
{
  ...
  for (i = 0; vulnerability_detector->queue_fd = StartMQ(DEFAULTQPATH, WRITE),
      vulnerability_detector->queue_fd < 0 && i < WM_MAX_ATTEMPTS;
       i++) {
    sleep(WM_MAX_WAIT);
  }
  ...
  if (vulnerability_detector->agents_triag = OSHash_Create(),
      !vulnerability_detector->agents_triag) {
    mterror(WM_VULNDETECTOR_LOGTAG, VU_CREATE_HASH_ERRO);
    pthread_exit(NULL);
  }

  OSHash_SetFreeDataPointer(vulnerability_detector->agents_triag,
                            (void (*)(void *))free_agents_triag);

  while (1) {
    // Update CVE databases
    if (flags->u_flags.update && wm_vuldet_updatedb(vulnerability_detector->updates)) {
      mterror(WM_VULNDETECTOR_LOGTAG, VU_OVAL_UPDATE_ERROR);
    }

    if ((vulnerability_detector->last_detection +
         (time_t)vulnerability_detector->detection_interval) < time(NULL)) {
      ...
      if (wm_vuldet_set_agents_info(&vulnerability_detector->agents_software, updates)) {
        ...
        if (wm_vuldet_check_agent_vulnerabilities(vulnerability_detector->agents_software,
                                                  vulnerability_detector->agents_triag,
                                                  vulnerability_detector->ignore_time)) {
    ...
    time_sleep =
        (vulnerability_detector->last_detection + vulnerability_detector->detection_interval) -
        t_now;
    if (time_sleep <= 0) {
      time_sleep = 1;
      i = OS_SUPP_SIZE;
...
cJSON *wm_vuldet_dump(const wm_vuldet_t *vulnerability_detector)
{
  ...
  if (vulnerability_detector->flags.enabled)
    cJSON_AddStringToObject(wm_vd, "disabled", "no");
  else
    cJSON_AddStringToObject(wm_vd, "disabled", "yes");
  if (vulnerability_detector->flags.run_on_start)
    cJSON_AddStringToObject(wm_vd, "run_on_start", "yes");
  else
    cJSON_AddStringToObject(wm_vd, "run_on_start", "no");
  ...
  for (i = 0; i < OS_SUPP_SIZE; i++) {
    if (vulnerability_detector->updates[i]) {
      ...
      if (vulnerability_detector->updates[i]->dist)
        cJSON_AddStringToObject(feed, "name", vulnerability_detector->updates[i]->dist);
      if (vulnerability_detector->updates[i]->version)
        cJSON_AddStringToObject(feed, "version", vulnerability_detector->updates[i]->version);
